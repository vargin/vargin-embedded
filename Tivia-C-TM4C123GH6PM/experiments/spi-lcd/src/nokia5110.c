...
void Nokia5110_Init(void){
  ...
  SSI0_CC_R = (SSI0_CC_R&~SSI_CC_CS_M)+SSI_CC_CS_SYSPLL;
                                        ...
                                        // clock divider for 3.571 MHz SSIClk (50 MHz PLL/14)
                                        // SysClk/(CPSDVSR*(1+SCR))
                                        // 50/(14*(1+0)) = 3.571 MHz (slower than 4 MHz)
  SSI0_CPSR_R = (SSI0_CPSR_R&~SSI_CPSR_CPSDVSR_M)+14; // must be even number

  SSI0_CR0_R &= ~(SSI_CR0_SCR_M |       // SCR = 0 (3.571 Mbps data rate)
                  SSI_CR0_SPH |         // SPH = 0
                  SSI_CR0_SPO);         // SPO = 0
  ...
  for(delay=0; delay<40; delay=delay+1);// delay minimum 100 ns
